...
namespace Hypermedia.JsonApi.AspNetCore
{
    internal sealed class JsonApiRequestMetadata<TResource> : IRequestMetadata<TResource>
    {
        ...
        public JsonApiRequestMetadata(IContractResolver contractResolver, IContract root, HttpRequest request)
        {
        ...
        {
            if (String.IsNullOrWhiteSpace(_request.Query["include"]))
            {
                return new MemberPath[0];
            }

            if (TryResolveInclude(_request.Query["include"], out var memberPaths) == false)
            {
                return new MemberPath[0];
            }
        ...
        bool TryResolveInclude(string path, out List<MemberPath> memberPaths)
        {
            ...
            {
                if (resolver.TryResolve(part.Trim(), out var memberPath) == false)
                {
                    return false;
                }
